...
/***/ "./renderer/react/SpreadsheetImport.utils.tsx"
/*!****************************************************!*\
  !*** ./renderer/react/SpreadsheetImport.utils.tsx ***!
  \****************************************************/
...
/* harmony export */   inferColumnType: () => (/* binding */ inferColumnType),
/* harmony export */   inferredTypeToPostgreSQLType: () => (/* binding */ inferredTypeToPostgreSQLType),
/* harmony export */   isObject: () => (/* binding */ isObject),
/* harmony export */   parseSpreadsheetText: () => (/* binding */ parseSpreadsheetText),
/* harmony export */   revertSpreadsheet: () => (/* binding */ revertSpreadsheet),
...
const inferColumnType = (column, rows) => {
    ...
    if (ISO_DATE_REGEX.test(str)) {
        const allDates = columnDataAcrossRows.every(item => {
            ...
            for (const fmt of DATE_FORMATS) {
                if (dayjs__WEBPACK_IMPORTED_MODULE_0___default()(s, fmt).isValid())
                    return true;
            }
            return false;
        });
    ...
    for (const fmt of DATE_FORMATS) {
        if (dayjs__WEBPACK_IMPORTED_MODULE_0___default()(str, fmt).isValid()) {
            const allDates = columnDataAcrossRows.every(item => {
                if (!item)
                    return true;
                const s = String(item).trim();
                for (const f of DATE_FORMATS) {
                    if (dayjs__WEBPACK_IMPORTED_MODULE_0___default()(s, f).isValid())
                        return true;
                }
                return false;
            });
...
// CSV Parsing
// ═════════════════════════════════════════════════════════════════════════════
function parseSpreadsheetText({ text, emptyStringAsNullHeaders, }) {
    const columnTypeMap = {};
    let previewRows = [];
    return new Promise((resolve) => {
        papaparse__WEBPACK_IMPORTED_MODULE_2___default().parse(text, {
            header: true,
            dynamicTyping: false,
            skipEmptyLines: true,
            complete: (results) => {
                ...
                const transformedRows = resolvedEmptyStringAsNullHeaders.length > 0
                    ? rows.map((row) => {
                        const rowAsObject = isObject(row) ? row : {};
                        return Object.fromEntries(Object.entries(rowAsObject).map(([k, v]) => [
                            k,
                ...
                headers.forEach((header) => {
                    ...
                    if (!(0,lodash__WEBPACK_IMPORTED_MODULE_1__.has)(columnTypeMap, header)) {
...
const revertSpreadsheet = (headers, rows) => {
    return papaparse__WEBPACK_IMPORTED_MODULE_2___default().unparse(rows, { columns: headers });
};
// ═════════════════════════════════════════════════════════════════════════════
// SQL Type Mapping
// ═════════════════════════════════════════════════════════════════════════════
const inferredTypeToPostgreSQLType = (type) => {
    const map = {
        int8: 'BIGINT',
        int4: 'INTEGER',
        float8: 'NUMERIC',
...
"use strict";
/*!*****************************************************!*\
  !*** ./renderer/react/SpreadsheetImportSidebar.tsx ***!
  \*****************************************************/
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   SpreadsheetImportSidebar: () => (/* binding */ SpreadsheetImportSidebar),
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! react */ "react");
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(react__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _SpreadsheetImport_utils__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./SpreadsheetImport.utils */ "./renderer/react/SpreadsheetImport.utils.tsx");
/**
...
`;
const SpreadsheetImportSidebar = ({ visible = false, selectedTable = null, onImport = () => { }, onClose = () => { }, }) => {
    const [tab, setTab] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)('upload');
    const [file, setFile] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(null);
    const [csvText, setCsvText] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)('');
    const [parsedData, setParsedData] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(null);
    const [selectedHeaders, setSelectedHeaders] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(new Set());
    const [columnTypeMap, setColumnTypeMap] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)({});
    ...
    const [debugLog, setDebugLog] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)([]);
    const fileInputRef = (0,react__WEBPACK_IMPORTED_MODULE_0__.useRef)(null);
    const styleRef = (0,react__WEBPACK_IMPORTED_MODULE_0__.useRef)(null);
    // Inject CSS
    ...
    const parseCSV = async (content) => {
        try {
            ...
            const parsed = await (0,_SpreadsheetImport_utils__WEBPACK_IMPORTED_MODULE_1__.parseSpreadsheetText)({
                text: content,
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            const types = headers.map((h) => columnTypeMap[h] || 'text');
            ...
            const columnDefs = headers
                .map((h, i) => `"${h}" ${(0,_SpreadsheetImport_utils__WEBPACK_IMPORTED_MODULE_1__.inferredTypeToPostgreSQLType)(types[i])}`)
                .join(', ');
            ...
            for (let batchIdx = 0; batchIdx < parsedData.rows.length; batchIdx += 100) {
                ...
                // Build INSERT SQL
                const placeholderRows = batchRows.map((_, rowIdx) => `(${headers.map((_, colIdx) => `$${rowIdx * headers.length + colIdx + 1}`).join(',')})`);
            ...
            if (totalErrorCount > 0)
                addLog(`⚠️ Fehler: ${totalErrorCount} Zeilen`);
            onImport();
    ...
    return (react__WEBPACK_IMPORTED_MODULE_0___default().createElement((react__WEBPACK_IMPORTED_MODULE_0___default().Fragment), null,
        react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "sidebar-overlay", onClick: onClose }),
        react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "sidebar-import" },
            react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "sidebar-header" },
                react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", null,
                    react__WEBPACK_IMPORTED_MODULE_0___default().createElement("h2", { className: "sidebar-header-title" }, "\uD83D\uDCCA CSV-Import"),
                    react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "sidebar-header-subtitle" }, "Intelligente Type-Erkennung")),
                react__WEBPACK_IMPORTED_MODULE_0___default().createElement("button", { className: "sidebar-close-btn", onClick: onClose, disabled: importing }, "\u2715")),
            react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "sidebar-tabs" }, ['upload', 'paste', 'preview'].map((t) => (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("button", { key: t, className: `sidebar-tab-btn ${tab === t ? 'active' : ''} ${t === 'preview' && !parsedData ? 'disabled' : ''}`, onClick: () => setTab(t), disabled: t === 'preview' && !parsedData },
                t === 'upload' && '📁 Upload',
                t === 'paste' && '📝 Paste',
                t === 'preview' && '👁 Preview')))),
            react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "sidebar-content" },
                tab === 'upload' && (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "upload-zone", onClick: () => fileInputRef.current?.click() },
                    react__WEBPACK_IMPORTED_MODULE_0___default().createElement("input", { ref: fileInputRef, type: "file", accept: ".csv", onChange: handleUploadFile, style: { display: 'none' } }),
                    react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "upload-icon" }, "\uD83D\uDCC1"),
                    react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "upload-title" }, "CSV-Datei hochladen"),
                    react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "upload-subtitle" }, "Oder klicken zum Ausw\u00E4hlen"),
                    file && react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "upload-success" },
                        "\u2713 ",
                        file.name))),
                tab === 'paste' && (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", null,
                    react__WEBPACK_IMPORTED_MODULE_0___default().createElement("textarea", { className: "textarea-input", value: csvText, onChange: (e) => setCsvText(e.target.value), placeholder: "CSV hier einf\u00FCgen (Komma-Separator, mit Header)" }),
                    react__WEBPACK_IMPORTED_MODULE_0___default().createElement("button", { className: "btn-parse", onClick: handleParsePasted }, "\uD83D\uDCDD Parsen & Vorschau"))),
                tab === 'preview' && parsedData && (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", null,
                    react__WEBPACK_IMPORTED_MODULE_0___default().createElement("label", { className: "section-label" },
                        "\uD83D\uDCCB SPALTEN (",
                        selectedHeaders.size,
                        "/",
                        parsedData.headers.length,
                        ")"),
                    react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "column-tags" }, parsedData.headers.map((h) => (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("label", { key: h, className: `column-tag ${selectedHeaders.has(h) ? 'selected' : ''}` },
                        react__WEBPACK_IMPORTED_MODULE_0___default().createElement("input", { type: "checkbox", checked: selectedHeaders.has(h), onChange: () => handleToggleHeader(h), style: { marginRight: '5px' } }),
                        h)))),
                    react__WEBPACK_IMPORTED_MODULE_0___default().createElement("label", { className: "section-label" }, "\uD83D\uDD0D SPALTEN-TYPEN"),
                    react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "type-grid" }, parsedData.headers.map((h) => (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { key: h, className: "type-field" },
                        react__WEBPACK_IMPORTED_MODULE_0___default().createElement("label", null, h.slice(0, 15)),
                        react__WEBPACK_IMPORTED_MODULE_0___default().createElement("select", { className: "type-select", value: columnTypeMap[h] || 'text', onChange: (e) => handleColumnTypeChange(h, e.target.value) }, typeOptions.map((t) => (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("option", { key: t, value: t }, t.toUpperCase())))))))),
                    react__WEBPACK_IMPORTED_MODULE_0___default().createElement("label", { className: "section-label" },
                        "\uD83D\uDC41 PREVIEW (",
                        Math.min(5, parsedData.rows.length),
                        " Zeilen)"),
                    react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { style: { overflowX: 'auto' } },
                        react__WEBPACK_IMPORTED_MODULE_0___default().createElement("table", { className: "data-preview-table" },
                            react__WEBPACK_IMPORTED_MODULE_0___default().createElement("thead", null,
                                react__WEBPACK_IMPORTED_MODULE_0___default().createElement("tr", null, parsedData.headers.slice(0, 8).map((h) => (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("th", { key: h }, h.slice(0, 12)))))),
                            react__WEBPACK_IMPORTED_MODULE_0___default().createElement("tbody", null, parsedData.rows.slice(0, 5).map((row, idx) => (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("tr", { key: idx }, parsedData.headers.slice(0, 8).map((h) => (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("td", { key: h, className: row[h] === null || row[h] === undefined || row[h] === '' ? 'null' : '' }, row[h] === null || row[h] === undefined ? '(null)' : String(row[h]).slice(0, 30))))))))))))),
            react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "debug-log" }, debugLog.map((log, idx) => (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { key: idx, className: `debug-log-entry ${log.includes('✓') ? 'success' : log.includes('❌') ? 'error' : log.includes('📤') || log.includes('✅') || progress ? 'progress' : ''}` }, log)))),
            react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "sidebar-footer" },
                react__WEBPACK_IMPORTED_MODULE_0___default().createElement("button", { className: "btn-secondary", onClick: onClose, disabled: importing }, "Abbrechen"),
                tab === 'preview' && parsedData && (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("button", { className: "btn-primary", onClick: handleImport, disabled: importing || selectedHeaders.size === 0 }, importing ? `📤 ${progress}` : `✓ Import`))))));
};
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (SpreadsheetImportSidebar);

})();

window.SpreadsheetImport = __webpack_exports__["default"];
/******/ })()
;
//# sourceMappingURL=spreadsheet-import.js.map